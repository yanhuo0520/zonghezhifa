  ...
  binding: function () {
    ...
    this.content.addEventListener('click', function (e) {
      if (e.target.tagName.toLowerCase() === 'li') {
		...
		other_id = li.getAttribute('other_id')  /************   增加other_id ***********/
        if (!li.classList.contains('ajaxPicker-content-on')) {
          ...
          _this['result' + (level + 1)] = {
            ...
            id: id,
            index: index,
			other_id: other_id      /************   增加other_id ***********/
          }
          _this.temResults.splice(level, 1, {value: value, id: id, index: index, other_id: other_id})  /************   增加index, other_id ***********/
  ...
  render: function (arr) {
    ...
    if (arr && arr.length) {
      ...
      loop(0, arr.length, function (i) {
        ...
        var id = arr[i].id
		var other_id = arr[i].index
        html += '<li class="ajaxPicker-content-item" level=' + '"' + _this.renderIdx + '" ' + 'index=' + '"' + i + '" ' + 'value=' + '"' + name + '" '+ 'other_id=' + '"' + other_id + '" ' + 'id=' + '"' + id + '"' + '>' + name + '</li>'
      })
      ul.innerHTML = html
